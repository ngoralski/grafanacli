...
class GrafanaAdmin(object):
    ...
    def OrganizationAddUser(self, orgId, loginOrEmail, role):
        json_data = {"loginOrEmail": "{0}".format(
            loginOrEmail), "role": "{0}".format(role)}
        run = self.Session.post(
            '{0}/api/orgs/{1}/users'.format(self.url, orgId), json=json_data)
        return(run.json())

    def OrganizationUpdateUser(self, orgId, userId, role):
        json_data = {"role": "{0}".format(role)}
        run = self.Session.patch(
            '{0}/api/orgs/{1}/users/{2}'.format(self.url, orgId, userId), data=json_data)
        return(run.json())

    def OrganizationDeleteUser(self, orgId, userId):
        run = self.Session.delete(
            '{0}/api/orgs/{1}/users/{2}'.format(self.url, orgId, userId))
        return(run.json())
                ...
                if key == 'Starred':
                    if type(kwargs[key]) == type(True):
                        url_search = url_search + \
                            '&starred={0}'.format(str(kwargs[key]).lower())
                    else:
    ...
    def DashboardDownload(self, DashboardUid, FileName):
        try:
            data = self.Session.get(
                '{0}/api/dashboards/uid/{1}'.format(self.url, DashboardUid)).json()
    ...
    def DashboardUpload(self, FileName, Overwrite=False, FolderId=0):
        ...
        json_data = {"folderId": FolderId,
                     "overwrite": Overwrite, "dashboard": json_content}
        run = self.Session.post('{0}/api/dashboards/db'.format(self.url), json=json_data)
        return(run.json())

    def DashboardUploadJson(self, JsonContent, Overwrite=False, FolderId=0):
        JsonContent['id'] = 'null'
        json_data = {"folderId": FolderId,
                     "overwrite": Overwrite, "dashboard": JsonContent}
        run = self.Session.post('{0}/api/dashboards/db'.format(self.url), json=json_data)
        return(run.json())

    def DashboardGet(self, DashboardUid):
        run = self.Session.get(
            '{0}/api/dashboards/uid/{1}'.format(self.url, DashboardUid))
        return(run.json())
    ...
    def GlobalUserUpdatePassword(self, userId, newPassword):
        ...
        run = self.Session.put(
            '{0}/api/admin/users/{1}/password'.format(self.url, userId), json=json_data)
        return(run.json())

    def GlobalUserGrafanaAdmin(self, userId, grafanaAdmin=False):
        json_data = {"isGrafanaAdmin": grafanaAdmin}
        run = self.Session.put(
            '{0}/api/admin/users/{1}/permissions'.format(self.url, userId), json=json_data)
        return(run.json())
    ...
    def GlobalUserSearch(self, loginOrEmail):
        run = self.Session.get(
            '{0}/api/users/lookup?loginOrEmail={1}'.format(self.url, loginOrEmail))
        return(run.json())
    ...
    def DatasourceUpdate(self, dsId, ds_json):
        run = self.Session.put(
            '{0}/api/datasources/{1}'.format(self.url, dsId), json=ds_json)
        return(run.json())
